...
/**
 * Activation function 
 * Creates the 'unverified' role type
 * 
 */
	...
	function wp_new_user_notification( $user_id, $plaintext_pass = '', $verify = true ) {
		...
		if ($verify) {
			...
			if ( empty( $plaintext_pass ) )
				return;
			$message  = __('Hi there,') . "\r\n\r\n";
			$message .= sprintf( __("Welcome to %s! click on the following link to login:"), $_SERVER['HTTP_HOST']) . "\r\n\r\n";
			//$host = 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST'];
...
//add_filter('rewrite_rules_array','sevRewriteRules');
function sevRewriteRules() {
	add_rewrite_rule( 'sev/(.*?)/(.+?)', 'index.php?email=$matches[1]key=$matches[2]', 'top');
}
...
add_action('plugins_loaded', 'sevInit');
 ...
 add_action('wp_ajax_registration_activation', 'sevRegistrationActivation');
 add_action('wp_ajax_nopriv_registration_activation', 'sevRegistrationActivation');
 function sevRegistrationActivation() {
     echo 'XFNGR';
     wp_die();
 }
 */
